from simmc.evaluate_action_prediction import evaluate_action_prediction
import torch
...
class CustomBertModel(torch.nn):
...
class SIMMCModel():
    def __init__(self, num_actions: int, num_attributes: int) -> None:
    ...
    def train(self, train_dataset, val_dataset, epochs, device, save: bool = True):
        warnings.simplefilter("ignore", UserWarning)

        optimizer = transformers.AdamW(
            self.bert.parameters(), lr=5e-5, weight_decay=1e-5)
        ...
        for epoch in range(epochs):
            ...
            go = tqdm(train_dataloader, unit='batch')
            go.set_description(f"Epoch {epoch}, LR {scheduler.get_last_lr()}")
            losses = []
            ...
            pprint(action_metrics)

        newScore = (action_metrics['action_accuracy'] +
                    action_metrics['attribute_accuracy']) / 2
        if save and newScore > score:
            score = newScore
            self.bert.save_pretrained('/content/drive/MyDrive/best_model')

    def validate(self, dataset, device):
        ...
        for batch in val_dataloader:
            ...
            with torch.no_grad():
                predict_action, predict_attributes = self.bert(b_input_ids, attention_mask=b_input_mask)
            ...
            predict_action = predict_action.to('cpu').numpy().tolist()
            predict_attributes = predict_attributes.to('cpu').numpy().tolist()
            actions_pred.extend(predict_action)
            attributes_pred.extend(predict_attributes > 0)

        with open(__valAPIFile) as gt_actions_json:
        ...
        attribute_predictions = [list(t) for t in mlb_attributes.inverse_transform(predict_attributes)]
