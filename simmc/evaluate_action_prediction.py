...
def evaluate_action_prediction(
            ...
            # Case 2: Action matches -- use model predictions for attributes.
                ...
                for key in supervision.keys():
                    ...
                    if isinstance(gt_key_vals, list):
                        ...
                        recall = np.mean([(ii in model_key_vals)
                                         for ii in gt_key_vals])
                        ...
                        matches["attributes"].append(
                            gt_key_vals == model_key_vals)
    ...
    if compute_std_err:
        metrics_std_err = {
            ...
            "attribute_accuracy": (
                np.std(matches["attributes"]) /
                np.sqrt(len(matches["attributes"]))
            ),
        }
